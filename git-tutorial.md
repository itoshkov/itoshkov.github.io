...
To fix this, let's create a list of files to ignore. Put the following in a
`.ignore` file:
...
The `-x@.ignore` option tells `zip` to exclude the files mentioned in the
`.ignore` file. Notice that `.ignore` is included in the archive. That's good.
We want to track its content with new versions.
